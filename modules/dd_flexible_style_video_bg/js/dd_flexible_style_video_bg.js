...
(function ($, Drupal, window, document, undefined) {
  ...
  Drupal.behaviors.dd_flexible_style_video_bg = {
    ...
    attach: function (context, settings) {
      ...
      // Store the current media query. We use this to determine when to
      // recreate the video.
      if (!self.current_mq) {
        self.current_mq = self.getCurrentMQ();
      }
      ...
      $(window).on('resize', function() {
        if (self.current_mq != self.getCurrentMQ()) {
          self.current_mq = self.getCurrentMQ();
          self.destroyVideoBg();
          self.createVideoBg();
        }
    ...
    getCurrentMQ: function () {
      ...
      for (var i = 0; i < breakpoints.length; i++) {
        if (window.matchMedia(Foundation.media_queries[breakpoints[i]]).matches) {
    ...
    createVideoBg: function () {
      ...
      $('.dd_flexible_style_video_bg').each(function () {
        ...
        $.each(video_bg_interchange, function (mq, videos) {
          if (window.matchMedia(Foundation.media_queries[mq]).matches) {
            if (videos.webm) {
              webm = videos.webm;
            }
            if (videos.mp4) {
